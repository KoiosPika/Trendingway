            ...
            <div className='w-full flex flex-col max-w-[1000px] justify-center items-center'>
                <div className='my-3 justify-center items-center flex flex-col w-full'>
                    <div className='rounded-lg flex flex-col justify-center items-center mt-3 mb-[100px] p-3 w-full lg:w-5/6 bg-white'>
                        <div className='w-full md:w-11/12 px-2 py-4 md:p-8 my-3 rounded-lg bg-[#1AAD7A] text-white' style={{ boxShadow: '0 8px 10px -6px gray, -8px 8px 8px -6px gray, 8px 8px 8px -6px gray' }}>
                            ...
                            <div className='grid grid-cols-1 gap-2'>
                                ...
                                {earningsData && earningsData.map((data: IEarningData, index: number) => (
                                    <div key={index} className='flex flex-row justify-center items-center p-2 gap-2 bg-white text-black font-bold rounded-lg'>
                                        ...
                                        <div className='w-full flex flex-row items-center gap-2'>
                                            <p className='text-[10px] lg:text-[15px]'>$ {Number((data?.total / 0.8).toFixed(2)).toLocaleString()}</p>
                                        </div>
                                        <div className='w-full flex flex-row items-center gap-2'>
                                            <p className='text-[10px] lg:text-[15px]'>$ {Number((data?.total).toFixed(2)).toLocaleString()}</p>
                                        </div>
                                        <div className='w-full flex flex-row items-center gap-2'>
                                            <p className='text-[10px] lg:text-[15px]'>{(data?.orderCount).toLocaleString()}</p>
                                        </div>
                                    </div>
                                ))}
                            </div>
